...
class Encoder(nn.Module):
    def __init__(
        ...
        image_channel: int = 3,
        image_size: int = 256,
        down_block_types: list[str] = ['DownBlock', 'DownBlock', 'AttnDownBlock', 'DownBlock'],
        ...
        assert all(db in ['DownBlock', 'AttnDownBlock'] for db in down_block_types), \
        f'down_block_types should contain "DownBlock" or "AttnDownBlock"'
        assert len(block_out_channels) == len(down_block_types), \
        ...
        assert block_out_channels[0] % num_groups == 0, \
